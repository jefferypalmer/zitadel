...
description: "Configure custom domains and solve 'Instance not found' errors by ensuring correct Host header routing for ZITADEL instances."
...
## Dynamic Client Registration: hostname-root requirement

If you intend to enable [OAuth 2.0 Dynamic Client Registration](/apis/openidoauth/dynamic-client-registration)
for Claude Code / MCP integrations, the deployment MUST run on a
**hostname-root issuer** — `https://example.com/`, NOT
`https://example.com/zitadel/`. MCP clients including Claude Code resolve
`registration_endpoint` against the issuer host, not the issuer path. The
`/.well-known/oauth-authorization-server` document mandated by RFC 8414
likewise lives at the host root, so a subpath issuer cannot serve it
correctly without a custom rewrite at the reverse proxy.

When DCR is enabled (`OIDC.DCR.Enabled: true`) and the configured
`ExternalDomain` resolves to a subpath, ZITADEL logs a structured
warning naming the probed `.well-known/oauth-authorization-server` URL
the **first time the AS-metadata endpoint is hit** (deduplicated per
`(instance, issuer)` tuple thereafter). Treat that warning as a
deployment defect: either move ZITADEL to the host root, or disable
DCR for this deployment.

This requirement does not affect any other ZITADEL feature; subpath
deployments remain fully supported for all non-DCR workflows.
