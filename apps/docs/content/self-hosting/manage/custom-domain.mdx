...
ZITADEL emits a **startup warning** naming the probed
`.well-known/oauth-authorization-server` URL when DCR is enabled
(`OIDC.DCR.Enabled: true`) and the configured `ExternalDomain` resolves
to a subpath. Treat the warning as a deployment defect: either move
ZITADEL to the host root, or disable DCR for this deployment.
