...
| T-014 | DONE (V1 only) | `domain.AuthRequest.Resources []string` field added. `CreateAuthRequestToBusiness` pulls from sidecar context and sets it. 3 subtests green. **Scope note:** V2 login path (`createAuthRequestLoginClient` → `command.AuthRequest` → `authrequest.NewAddedEvent`) NOT covered. Adding Resources there requires event-schema changes in the authrequest aggregate — intentionally deferred, needs its own task. Impact: DCR-registered clients using the V2 login flow will NOT carry their `resource` indicator through until the V2 gap is closed. Flag for follow-up. |
