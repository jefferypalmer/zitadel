...
- [ ] **`ClientSecretExpiresIn` non-negative (added 2026-04-27 / F-300).** Positive `time.Duration` = client_secret expires `IssuedAt + ClientSecretExpiresIn`. `0` = "no expiry" sentinel (RFC 7591 §3.2.1). NEGATIVE values MUST cause startup to refuse with a clear field-named error — a negative lifetime would advertise a freshly-issued secret as already expired, breaching RFC 7591 §3.2.1.
- [ ] **`MaxRequestBodyBytes` sentinel semantics (added 2026-04-27 / F-204).** Positive integer = enforced cap. `0` is INVALID — startup MUST refuse with an error naming the field rather than silently substituting the package default. To DISABLE the per-request cap (e.g. for clients posting large `software_statement` JWTs), the operator MUST set `-1`. Any other negative value is invalid and MUST cause startup to refuse.
- [ ] **`MaxRequestBodyBytes` defence-in-depth ceiling (added 2026-04-27 / F-301).** Even when `-1` is configured, the decoder MUST enforce a hard package-level upper bound — `dcr.AbsoluteMaxBodyBytes = 100 MiB`. Bodies exceeding the absolute ceiling MUST return 413 `payload_too_large`. The `-1` sentinel disables the operator-tunable per-request cap but NOT the absolute safety net. Rationale: an unauthenticated POST endpoint with no upper bound is a memory-exhaustion DoS one config typo away.
- [ ] **Positive `MaxRequestBodyBytes` > ceiling refusal (added 2026-04-27 / F-402).** When `MaxRequestBodyBytes` is positive AND > `dcr.AbsoluteMaxBodyBytes`, startup MUST refuse with a field-named error rather than silently runtime-clamping. Mirrors the F-204 precedent (operator-set value the package cannot honour is a startup-refuse condition, not a runtime-substitute condition). Silent clamping mis-leads operators when 413 envelopes reference a value not present in their config.
- [ ] **Startup WARN on `-1` (added 2026-04-27 / F-301).** When `MaxRequestBodyBytes=-1` is configured, startup MUST emit a WARN log naming `OIDC.DCR.MaxRequestBodyBytes` and the absolute ceiling value, so an operator who accidentally sets `-1` sees the warning at boot.
